...
**Current state:** the **matching engine** (pure-domain pipeline, `src/emule_indexer/domain/matching/`) and the **EC adapter** (`src/emule_indexer/adapters/mule_ec/`: pure sync codec, async transport, `AmuleEcClient` — auth/search/status — mapping to `FileObservation`, plus the `tools/ec_probe.py` CLI and a mandatory testcontainers integration suite against a real `amuled`) are built. Key empirical fact: **EC exposes no media metadata on search results** (see `docs/reference/2026-06-11-ec-field-richness.md`). The SQLite data model, search orchestration, auto-download/verifier, observability, and packaging are **not built yet**. Milestones are tagged `v0.1.0-foundations` → `v0.5.0-ec-adapter` on `main` (not pushed). Integration tests: `uv run pytest -m ec_integration --no-cov` (Docker required; deselected from the default run).
