...
### Task 1 : DTO `SharedFileEntry` + méthode du port
...
Et faire que `_StubDownloadClient` (déjà présent) satisfasse le Protocol étendu en lui ajoutant :

```python
    async def shared_files(self) -> tuple[SharedFileEntry, ...]:
        return (SharedFileEntry(ed2k_hash="a" * 32, name="x.avi"),)
```
...
Dans `ports/mule_download_client.py`, ajouter le DTO après `DownloadEntry` :
...
Et ajouter au Protocol `MuleDownloadClient` la méthode :
...
git commit -m "feat(download): DTO SharedFileEntry + méthode port shared_files"
```
...
### Task 3 : `AmuleEcClient.shared_files()`
...
Run: `( cd packages/crawler && uv run pytest tests/adapters/mule_ec/test_client_download.py --no-cov -q )`
Expected: PASS (tous, anciens + nouveaux).

- [ ] **Step 5 : Commit**

```bash
git add packages/crawler/src/emule_indexer/adapters/mule_ec/client.py packages/crawler/tests/adapters/mule_ec/test_client_download.py
git commit -m "feat(ec): AmuleEcClient.shared_files (GET_SHARED_FILES → SHARED_FILES)"
```
